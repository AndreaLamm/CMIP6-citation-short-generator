...
def _get_path_processor(drs):
    drs_funcs = {
        "CMIP6output_exp": {
            "subject": _get_subject_cmip6output_path_exp,
            "ids": _get_ids_cmip6output_path,
        },
        "CMIP6output_source": {
            "subject": _get_subject_cmip6output_path_source,
            "ids": _get_ids_cmip6output_path,
        },
        "CMIP6input4MIPs": {
...
def _get_subject_cmip6output_path_exp(path):
...
def _get_subject_cmip6output_path_source(path):
    ids = _get_ids_cmip6output_path(path)

    return "{mip_era}.{activity_id}.{institution_id}.{source_id}".format(
        **ids
    )
